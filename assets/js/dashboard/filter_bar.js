...
export class FilterBar extends dc.BaseMixin {
  constructor(parent, dataFilter, group) {
    ...
    this._parent = parent;
    this.elTemplate = $('.chart-filter', this._parent)[2];
  ...
  drawFilter(filter) {
    if (!this._filters[filter.label]) {
      ...
      const newFilter = this.elTemplate.cloneNode(true);
      const el = $(newFilter);
  ...
  render() {
    $(this._parent).find('.chart-filter:not(.chart-filter--default)').remove()
